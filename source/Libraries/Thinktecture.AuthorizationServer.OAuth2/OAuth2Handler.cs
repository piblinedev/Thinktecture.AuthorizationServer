...
namespace Thinktecture.AuthorizationServer.OAuth2
{
    ...
    {
        private readonly HttpAuthentication _authN;

        public const string PrincipalKey = "TT_Principal";

        public OAuth2Handler(AuthenticationConfiguration configuration,
                                     HttpConfiguration httpConfiguration = null)
            : this(new HttpAuthentication(configuration), httpConfiguration)
        {
        }
        ...
        public OAuth2Handler(AuthenticationConfiguration configuration, HttpMessageHandler innerHandler)
            : this(new HttpAuthentication(configuration), innerHandler)
        {
        }
                                                                     ...
                                                                     CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                ...
                Tracing.ErrorFormat("Exception while validating the token: ", ex);
                return SendUnauthorizedResponse(request);
            }
        ...
        private HttpResponseMessage SendAuthenticationExceptionResponse(AuthenticationException aex,
